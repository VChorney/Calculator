...
public class CountMatrix extends CountOrderedData implements Clean<Double> {
    public CountMatrix(Double[][] firstMatrix, Double[][] secondMatrix, Double[][] resultMatrix) {
    ...
    public void expoonent(int exponent, int row, int column, Double[][] matrix){
            ...
            for (int t=1;t<exponent;t++){
                ...
                setResultList(multiplication(row,column,row,column,doubles,matrix));
                buffer=clean(buffer,row,column,0.0);
                for(int i=0;i<row;i++){
                    if (column >= 0) System.arraycopy(getResultList()[i], 0, buffer[i], 0, column);
                }
        ...
        public void revers(int size, Double[][]matrix){
            ...
            setResultList(clean(getResultList(),size,size,0.0));
